...
namespace Tac.Perceptron
{
	...
	public class NeironNetTreeOld
	{
		...
		public NeironNetTreeOld(int argSCount, int argACount, int argRCount, int argHCount, int argECount, string argName = "")
		{
		...
		protected void RActivation(int argStimulNumber)
		{
			...
			for (int j = 0; j < RCount; j++)
			{
				for (int i = 0; i < ACount; i++)
				{
					if (Activations[argStimulNumber][i] > 0)
					{
		...
		protected void LearnedStimul(int argStimulNumber)
		{
			for (int j = 0; j < RCount; j++)
			{
				for (int i = 0; i < ACount; i++)
				{
					if (Activations[argStimulNumber][i] > 0)
					{
						WeightAR[i][j] += (int)ReactionError[j];
					}
				}
			}
		}
